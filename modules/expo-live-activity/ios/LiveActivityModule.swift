...
public class LiveActivityModule: Module {
    public func definition() -> ModuleDefinition {
        ...
        AsyncFunction("startActivity") { (activityName: String,
                                         duration: Double,
                                         deepLink: String) async throws -> String in
          guard ActivityAuthorizationInfo().areActivitiesEnabled else {
            throw NSError(domain: "LiveActivity", code: 1,
                          userInfo: [NSLocalizedDescriptionKey: "Live Activities are not enabled"])
          }

          let attributes = LiveActivityAttributes(activityName: activityName,
                                                  deepLink: deepLink)
        ...
        AsyncFunction("pauseActivity") { (activityId: String?) async -> Bool in
            ...
            guard currentState.isRunning() else { return false }

            let pausedState = currentState.pauseTimer()
            let content = ActivityContent(state: pausedState, staleDate: pausedState.getFutureDate())
        ...
        AsyncFunction("resumeActivity") { (activityId: String?) async -> Bool in
            ...
            let currentState = activity.content.state
            guard !currentState.isRunning() && !currentState.isCompleted() else { return false }

            let resumedState = currentState.resumeTimer()
            let content = ActivityContent(state: resumedState, staleDate: resumedState.getFutureDate())
        ...
        AsyncFunction("updatePayload") { (activityId: String, payload: [String: Any]) async -> Bool in
          ...
          let newState = LiveActivityAttributes.ContentState(
            ...
            totalPausedTime: accumulatedPaused
          )
