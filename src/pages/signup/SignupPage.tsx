...
import SignupButton, {
  AgreeValue,
} from "@/src/pages/signup/submit/SignupButton";
...
function SignupPage({ token, provider }: { token: string; provider: string }) {
  ...
  const [focusedInput, setFocusedInput] = useState<ButtonState | null>(
    ...
    <View style={{ width: "80%", height: "100%", alignSelf: "center" }}>
      ...
      {inputNames.has(ButtonState.NICKNAME) && (
        <TextInputTemplate
          ...
          onChangeValue={(text) => {
            setNickname(text);
            if (
              userSchema.pick({ nickname: true }).safeParse({ nickname: text })
                .success
            ) {
              setIsNicknameValid(true);
            } else {
      ...
      {inputNames.has(ButtonState.TERMS_AND_CONDITIONS) && (
        <SignupButton
          ...
          isFocused={
            focusedInput === ButtonState.TERMS_AND_CONDITIONS && isScreenFocused
          }
