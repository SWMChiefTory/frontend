...
import { useDeleteUserViewModel, useLogoutViewModel } from "@/src/modules/user/business/service/useAuthService";
...
export default function AuthActions() {
  ...
  const handleLogoutPress = () => {
    Alert.alert("로그아웃", "정말 로그아웃 하시겠습니까?", [
      ...
      {text: "로그아웃", onPress: () => {
        logout();
        Alert.alert("로그아웃이 완료되었습니다.", "로그인 화면으로 이동합니다.", [
          {text: "확인", onPress: () => {
            completeLogout();
          }}
        ]);
  ...
  return (
     <View style={styles.userAcessContainer}>
       <AuthActionTemplate authAction={handleLogoutPress} actionName="로그아웃" />
       <AuthActionTemplate authAction={handleDeleteUserPress} actionName="회원탈퇴" />
     </View>
  );
}
