...
import { extractAxiosErrorInfo } from "@/src/modules/shared/utils/axiosUtils";
...
export const getUser: () => Promise<User> = async () => {
  ...
  const response = await proxyServerClient.post("/api/user/me", {
    accessToken,
  });
...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const signin = async ({ provider_token, provider }: LoginInfo) => {
    return await proxyServerClient
      .post(LOGIN_PATH, {
        provider: provider,
      ...
      .catch((err) => {
        if (err instanceof AxiosError) {
          const errorInfo = extractAxiosErrorInfo(err);
          if (
            errorInfo.code === StatusCodes.UNAUTHORIZED &&
            errorInfo.error === "USER_NOT_FOUND"
          ) {
  ...
  const signup = async (signupData: SignupData) => {
    try {
      const response = await proxyServerClient.post(SIGNUP_PATH, signupData);
