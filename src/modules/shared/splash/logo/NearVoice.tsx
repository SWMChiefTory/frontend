...
import Animated, { FadeIn, SharedValue, useAnimatedStyle, withDelay, withTiming } from "react-native-reanimated";
import logoStyle from "./style/logostyle";
import { useSharedValue } from "react-native-reanimated";
import { useEffect } from "react";

function NearVoice({translateY, fadeStart, fadeDuration }: {translateY: SharedValue<number>, fadeStart:number, fadeDuration:number}) {
  const translateNearOpacity = useSharedValue(0);
  useEffect(() => {
    translateNearOpacity.value = withDelay(fadeStart, withTiming(1, { duration: fadeDuration }));
  }, []);
